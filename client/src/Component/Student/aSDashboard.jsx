...
export default function Dashboard() {
  ...
  const markHead = ["Subject","Syllabus", "Test 1", "Test 2", "Exam", "Performance"];
  ...
  useEffect(async () => {
    ...
    if (c) {
      ...
      localStorage.setItem('notiFicationData',JSON.stringify(notiData.data) )
  ...
  async function getMyLectureFun() {
    ...
    subjects.map(async (item, i) => {
      ...
      console.log("iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii", myLectureData.data)
      setAttendMyDetails([...myAttendDetails, myLectureData.data]);
      arr[i] = myLectureData.data;
    });

    setAttendMyDetails(arr);
  ...
  async function onChangeHandler(id) {
    ...
    const dataOneNotificaton = await getOneSyllabus(`${id},${myDetails.ssemester},${myDetails.sdept}`);
              ...
              <div className="fun-work my-notify-scroll">
                <div className="st-studentInfo box-shadow">
                  <div className="st-pic ">
                    ...
                    />
                    <br />
                    <span className="text-shadow">{myDetails.sname ? myDetails.sname.split(' ')[0].toUpperCase() : ""}</span> <br />
                    ...
                    <Link to="./">
                      {" "}
                      <button className="btn btn-light box-shadow" onClick={logout}>
                ...
                <button className="btn btn-primary box-shadow btn-shadow" onClick={getMyLectureFun}>
                      ...
                      <table className="table table_striped box-shadow">
                        ...
                        <tbody>
                          {myAttendDetails.map((item, index) => {
                            console.log("valuse",subjectsValues)
                            console.log("valuse",subjects)
                            console.log("valuse",myAttendDetails)
                            const e = subjectsValues[index].attendance;

                            const att =
                              (subjectsValues[index].attendance /
                                item.sub.lec) *
                              100;
                          ...
                          return (
                            <>
                              <tr>
                                ...
                                <td> <a
                                className="zoom"
                    onClick={() => onChangeHandler(subjects[index])}
...
<div className=" ">
<div>
<div className="chart"><h3>Marks</h3><Line/></div>
<div className="chart"><h3>Attendance</h3><Pie/></div>
<div className="chart"><h3>Marks</h3><Donut/></div>
</div>
