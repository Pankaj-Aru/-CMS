...
export default function Fdashboard() {
  ...
  const [counterr, setCounterr] = useState({facultyCount:0,studentCount:0,users:0,adminCount:0});
  ...
  useEffect(async () => {
   ...
   console.log("asdsdasas>>>>>>>dd", counter);
   ...
   if(counter.status===200){
    ...
    setCounterr({facultyCount:dt.facultyCount*per,studentCount:dt.studentCount*per,users:dt.users,adminCount:dt.adminCount*per})
   
    console.log("asdsdasas>>>>>>>dd", counterr);
  ...
  const setForm = async (data, e) => {
    ...
    if (data === "faculty") {
      ...
      const isFacultyFormSet=await facultyFormSet({aPRN:user.aPRN,faculty:e.target.checked,student:user.isStudentAllow})
      ...
      const isStudentFormSet=await facultyFormSet({aPRN:user.aPRN,faculty:user.isFacultyAllow,student:e.target.checked})
  ...
  return (
    <>
              ...
              <div>
                <div className="text-shadow " style={{float:'right', fontWeight:'bolder'}}>
                  <h4>
